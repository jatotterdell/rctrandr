...
#' @return A list
complete_rand <- function(target_alloc,
                          sample_size) {
  # Normalise weights
...
#' @param target_alloc
#' @param sample_size
restricted_rand <- function(target_alloc,
                            sample_size) {
...
#' @return A list
permuted_block_rand <- function(target_alloc,
                                sample_size,
                                blocksize) {
...
#' @param blocksize The size of each block
#' @return A list
#' @references Zhao, W., & Weng, Y. (2011). Block urn design—a new randomization algorithm for 
#' sequential trials with two or more treatments and balanced or unbalanced allocation. 
#' Contemporary clinical trials, 32(6), 953-961.
block_urn_rand <- function(target_alloc,
                           sample_size,
                           blocksize) {
  # RUN ARGUMENT CHECKS...
  arms <- length(target_alloc)
...
#' @param sample_size The number of allocations to generate
#' @return A list
modified_wei_urn_rand <- function(target_alloc,
                                  sample_size,
                                  alpha,
                                  beta) {
...
#' @references Zhao (2015) Mass weighted urn design--A new randomization algorithm for unequal allocations. 
#' Contemporary clinical trials, 43, 209–216. https://doi.org/10.1016/j.cct.2015.06.008
mass_weighted_urn_rand <- function(target_alloc,
                                   sample_size,
                                   alpha = 4) {
  arms <- length(target_alloc)
